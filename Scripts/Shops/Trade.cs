...
namespace Shops
{
    public class Trade
    {
        ...
        {
            return IsEnoughResources(buyer, _input) && IsEnoughResources(seller, _output);
        }

        private bool IsEnoughResources(Inventory inventory, Dictionary<string, int> resouces)
        {
            foreach (KeyValuePair<string, int> item in resouces)
            {
                Item tmp = inventory.GetItem(item.Key, item.Value);
